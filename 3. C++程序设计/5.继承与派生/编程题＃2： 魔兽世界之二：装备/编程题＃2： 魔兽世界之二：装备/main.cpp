...
//  编程题＃2： 魔兽世界之二：装备
//
//  Created by LuJiayi on 2/19/16.
//  Copyright © 2016 YUCEN CAO. All rights reserved.
//
...
void base:: monsfucation(){
    string weapon[3] = {"sword","bomb","arrow"};
    if(monsname[order[count]] == "dragon"){
        cout << "It has a " << weapon[(totaltime + 1)%3] << ",and it's morale is " <<  setprecision(2) << fixed<<  double(energy )/double(monsblood[order[count]] )<< endl;
    }
    if(monsname[order[count]] == "ninja"){
        cout << "It has a "<< weapon[(totaltime + 1)%3] <<" and a "<< weapon[(totaltime + 2)%3]  << endl;
    }
    if(monsname[order[count]] == "iceman"){
        cout << "It has a " << weapon[(totaltime + 1)%3] << endl;
    }
    if(monsname[order[count]] == "lion"){
        cout << "It's loyalty is " << energy << endl;
    }
